...
// Save the instance
b.data(a,i,this)}var h=a.document,i="__pz__",j=Array.prototype.slice,k=!!a.PointerEvent,l=function(){var a=h.createElement("input");return a.setAttribute("oninput","return"),"function"==typeof a.oninput}(),m=/([A-Z])/g,n=/^http:[\w\.\/]+svg$/,o=/^inline/,p="(\\-?[\\d\\.e]+)",q="\\,?\\s*",r=new RegExp("^matrix\\("+p+q+p+q+p+q+p+q+p+q+p+"\\)$");/**
...
return q&&(d=this._checkDims(),e=this.container,l=d.width+d.widthBorder,m=d.height+d.heightBorder,f=l*Math.abs(n)>e.width?(l*Math.abs(n)-e.width)/2:0,g=m*Math.abs(n)>e.height?(m*Math.abs(n)-e.height)/2:0,j=d.left+d.margin.left,k=d.top+d.margin.top,"invert"===q?(h=l>e.width?l-e.width:0,i=m>e.height?m-e.height:0,f+=(e.width-l)/2,g+=(e.height-m)/2,a[4]=Math.max(Math.min(a[4],f-j),-f-j-h),a[5]=Math.max(Math.min(a[5],g-k),-g-k-i+d.heightBorder)):(f+=d.widthBorder/2,g+=d.heightBorder/2,h=e.width>l?e.width-l:0,i=e.height>m?e.height-m:0,"center"===p.css("textAlign")&&o.test(b.css(this.elem,"display"))?h=0:f=g=0,a[4]=Math.min(Math.max(a[4],f-j),-f-j+h),a[5]=Math.min(Math.max(a[5],g-k),-g-k+i))),"skip"!==c.animate&&this.transition(!c.animate),c.range&&this.$zoomRange.val(n),this.setTransform("matrix("+a.join(",")+")"),c.silent||this._trigger("change",a),a}},/**
...
_resetStyle:function(){this.$elem.css({cursor:"",transition:""}),this.$parent.css({overflow:"",position:""})},/**
		 ...
		 */
_bind:function(){var a=this,c=this.options,d=c.eventNamespace,e=k?"pointerdown"+d:"touchstart"+d+" mousedown"+d,f=k?"pointerup"+d:"touchend"+d+" click"+d,h={},i=this.$reset,j=this.$zoomRange;
...
step:c.rangeStep===g.defaults.rangeStep&&j.attr("step")||c.rangeStep,min:c.minScale,max:c.maxScale}).prop({value:this.getMatrix()[0]}),!c.disableZoom){var m=this.$zoomIn,n=this.$zoomOut;
// Bind zoom in/out
// Don't bind one without the other
m.length&&n.length&&(
// preventDefault cancels future mouse events on touch events
m.on(f,function(b){b.preventDefault(),a.zoom()}),n.on(f,function(b){b.preventDefault(),a.zoom(!0)})),j.length&&(h={},h[(k?"pointerdown":"mousedown")+d]=function(){a.transition(!0)},h[(l?"input":"change")+d]=function(){a.zoom(+this.value,{noSetRange:!0})},j.on(h))}},/**
		 * Unbind all events
		 */
...
_trigger:function(a){"string"==typeof a&&(a="panzoom"+a),this.$elem.triggerHandler(a,[this].concat(j.call(arguments,1)))},/**
		 ...
		 */
_startMove:function(a,d){var e,f,g,i,j,l,m,n,o=this,p=this.options,q=p.eventNamespace,r=this.getMatrix(),s=r.slice(0),t=+s[4],u=+s[5],v={matrix:r,animate:"skip"};
// Use proper events
k?(f="pointermove",g="pointerup"):"touchstart"===a.type?(f="touchmove",g="touchend"):(f="mousemove",g="mouseup"),f+=q,g+=q,this.transition(!0),this.panning=!0,this._trigger("start",a,d),d&&2===d.length?(i=this._getDistance(d),j=+r[0],l=this._getMiddle(d),e=function(a){a.preventDefault();var b=o._getMiddle(d=a.touches),c=o._getDistance(d)-i;o.zoom(c*(p.increment/100)+j,{focal:b,matrix:r,animate:!1}),o.pan(+r[4]+b.clientX-l.clientX,+r[5]+b.clientY-l.clientY,v),l=b}):(m=a.pageX,n=a.pageY,e=function(a){a.preventDefault(),o.pan(t+a.pageX-m,u+a.pageY-n,v)}),b(h).off(q).on(f,e).on(g,function(a){a.preventDefault(),b(this).off(q),o.panning=!1,a.type="panzoomend",o._trigger(a,r,!c(r,s))})}},b.Panzoom=g,b.fn.panzoom=function(a){var c,d,e,f;
